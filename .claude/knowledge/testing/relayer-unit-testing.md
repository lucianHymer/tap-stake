# Relayer Unit Testing Strategy

## Overview
The EIP-7702 relayer uses unit tests with stubbed blockchain interactions rather than full integration tests, optimizing for speed and CI-friendliness.

## Test Strategy
- **Stub viem client methods**: createWalletClient, createPublicClient, recoverAuthorizationAddress
- **Mock balance checks**: Return 100 tokens
- **Mock transaction submission**: Return '0xmocktxhash'
- **Focus**: Request validation logic, not viem internals or actual blockchain behavior
...
- **EIP-7702 limitations**: Can't be fully tested locally anyway (Foundry doesn't support it)

## Test Suite Coverage
32 tests across HTTP, validation, operations, auth, and config

**Related files**: packages/relayer/src/index.test.ts, packages/relayer/vitest.config.ts, packages/relayer/TEST_README.md
